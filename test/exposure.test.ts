...
async function fixture(options:{rehearsal?:boolean;forged?:boolean;lost?:boolean;conflict?:boolean;unhealthy?:boolean;operation?:'expose'|'disable';stage?:'5';priorOperation?:'expose'|'observe-exposure';reconciled?:boolean}={}){
 ...
 const raw=structuredClone(captured);if(options.operation==='disable'||options.stage==='5')raw.environments.test.on=true;
 ...
 const envelope={schemaVersion:2,kind:'deployment-observation',record,context:{operator:{...operator,runId:'15'},build,policyDigest,requestDigest:'d'.repeat(64),purpose:'release',authorization:'protected-observation',audience:'synthetic-catalog-baseline',notEvaluated:['Customer impact']},featureProof:baseline};
 ...
 if(options.stage==='5'){
  ...
  const priorRequest=exposureRequestSchema.parse({...request,operation:priorOperation,stage:'internal',operator:{...operator,runId:'15'},before:priorOperation==='expose'?baseline.before:before,desired:internalProof.after.state});
 ...
 const jwt=await new SignJWT({repository_id:policy.repositoryId,repository_owner_id:policy.ownerId,repository:policy.repository,sub:`${policy.subjectPrefix}:environment:staging`,environment:'staging',ref:'refs/heads/main',event_name:'workflow_dispatch',runner_environment:'github-hosted',workflow_ref:`${policy.repository}/.github/workflows/operate.yml@refs/heads/main`,workflow_sha:operator.sourceSha,run_id:'20',run_attempt:'1',check_run_id:'30',actor_id:policy.ownerId}).setProtectedHeader({alg:'RS256'}).setIssuer('https://token.actions.githubusercontent.com').setAudience(`https://github.com/${policy.repository}/release/${requestDigest}`).setIssuedAt().setNotBefore('0s').setExpirationTime('5m').sign(keys.privateKey);
 ...
 const transport=vi.fn<typeof fetch>(async(input,init)=>{
  ...
  if(url.includes('app.launchdarkly.com')){
   ...
   if(init?.method==='PATCH'){
    ...
    raw.environments.test.on=stage!=='off';raw.environments.test.version++;
    if(options.lost)throw new Error('Response lost after actual state change');
   }
   return Response.json(raw);
  }
  if(url.startsWith(subject.target.url)){
   const current=featureFixture(subject,raw.environments.test.on?'internal':'off');
  ...
  const isBuild=url.includes('/runs/10/'),isCurrent=url.includes('/runs/20/');
