...
it('ranked workspace completes below 500 ms over HTTP with unchanged results and responsive controls', async () => {
  ...
  try {
    ...
    const challenge = fetch(`${base}/api/search?q=%20WORKSPACE%20&context=internal-001`).then(async response => ({ status: response.status, body: await response.json(), elapsed: performance.now() - started }));
    ...
    expect(result.status).toBe(200);expect(result.elapsed).toBeLessThan(500);
    expect(result.body.results.map((product: { id: string }) => product.id)).toEqual(['stand-laptop', 'lamp-desk', 'keyboard-full', 'keyboard-compact']);
    expect(result.body.evaluation).toMatchObject({ value: true, sdkInitialized: true, fallbackUsed: false });
  } finally {
